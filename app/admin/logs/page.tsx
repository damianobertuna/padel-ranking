    ...
    const renderPagination = (total: number, current: number) => {
        ...
        const renderButton = (page: number, label: string | number, title: string, disabled: boolean, isActive: boolean = false) => {
            ...
            if (disabled || isActive) {
                return (
                    <button key={`log-page-${label}`} type="button" disabled={true} title={title} className={buttonClass}>
                        {label}
                    </button>
                );
            }
            ...
            return (
                <Link key={`log-page-${label}`} href={buildPaginationUrl(page)} scroll={false} title={title} className={buttonClass}>
                    {label}
                </Link>
            );
        };
            ...
            <div className="max-w-4xl w-full">
                ...
                <div className="bg-white border border-slate-200 rounded-sm shadow-sm overflow-hidden">
                    <div className="hidden sm:grid grid-cols-12 gap-4 px-4 py-3 bg-slate-50 border-b border-slate-200 text-[9px] font-black text-slate-500 uppercase tracking-widest">
                        <div className="col-span-2">{t('admin', 'LOG_COL_DATE')}</div>
                        <div className="col-span-3">{t('admin', 'LOG_COL_ACTION')}</div>
                        <div className="col-span-7">{t('admin', 'LOG_COL_DETAILS')}</div>
                    </div>
                            ...
                            {logs.map((log) => (
                                <div key={log.id} className="grid grid-cols-1 sm:grid-cols-12 gap-2 px-4 py-3 items-center hover:bg-slate-50 transition-colors">
                                    ...
                                    <div className="col-span-7 text-[11px] font-bold text-slate-900">
                                        {log.details}
                                        <span className="block text-[9px] font-black text-slate-400 uppercase mt-0.5">{t('admin', 'LOG_OPERATOR', { name: log.admin_name })}</span>
                                    </div>
                                </div>
                            ))}
                        </div>
