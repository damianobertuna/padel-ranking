...
export default function EditProfileForm({ player }: EditProfileFormProps) {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            setMessage({ type: 'success', text: dictProfile.SAVE_SUCCESS });
        } catch (error: any) {
    ...
    return (
        <form onSubmit={handleSubmit} className="bg-white p-6 rounded-sm shadow-sm border border-slate-200">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                <div>
                    <label htmlFor="firstName" className="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1.5">{dictProfile.LABEL_NOME}</label>
                ...
                <div>
                    <label htmlFor="lastName" className="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1.5">{dictProfile.LABEL_COGNOME}</label>
                ...
                <div>
                    ...
                    >
                        <option value="Right">{dictAuth.OPTION_SIDE_RIGHT}</option>
                        <option value="Left">{dictAuth.OPTION_SIDE_LEFT}</option>
                        <option value="Both">{dictAuth.OPTION_SIDE_BOTH}</option>
                    </select>
                </div>

                <div>
                    <label htmlFor="dominantHand" className="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1.5">
                        {dictProfile.LABEL_MANO}
                    </label>
                    <select
                        id="dominantHand"
                        name="dominantHand"
                        ...
                        className="w-full h-[42px] px-3 bg-slate-50 border border-slate-200 rounded-sm font-bold text-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-600 focus:border-transparent transition-all appearance-none"
                    >
                        <option value="Destro">{dictAuth.OPTION_HAND_RIGHT}</option>
                        <option value="Mancino">{dictAuth.OPTION_HAND_LEFT}</option>
                    </select>
                </div>
            </div>
            ...
            <div className="flex justify-end">
                <button type="submit" disabled={isSaving} className="h-[42px] px-6 bg-slate-900 hover:bg-slate-800 text-white font-black text-xs uppercase tracking-widest rounded-sm transition-colors disabled:opacity-50 flex items-center justify-center min-w-[120px]">
                    {isSaving ? <span className="animate-pulse">{dictProfile.BUTTON_SAVING}</span> : dictProfile.BUTTON_SAVE}
                </button>
            </div>
        </form>
    );
